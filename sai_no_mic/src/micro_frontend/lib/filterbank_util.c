...
#else
  state->weights = malloc(weight_index_start * sizeof(*state->weights));
	mymemset(state->weights, 0, weight_index_start * sizeof(*state->weights));
  state->unweights = malloc(weight_index_start * sizeof(*state->unweights));
	mymemset(state->unweights, 0, weight_index_start * sizeof(*state->unweights));
#endif
