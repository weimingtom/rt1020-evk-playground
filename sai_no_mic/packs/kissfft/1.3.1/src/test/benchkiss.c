...
int main(int argc,char ** argv)
{
        ...
        switch (c) {
            ...
            case 'n':
                ...
                if (nfft[0] != kiss_fft_next_fast_size(nfft[0]) ) {
                    ...
                    fplintf(stderr,"warning: %d might be a better choice for speed than %d\n",ng,nfft[0]);
                }
                break;
            case 'x':
                numffts = atoi (optarg);
...
#ifdef USE_SIMD        
    numffts /= 4;
    fplintf(stderr,"since SIMD implementation does 4 ffts at a time, numffts is being reduced to %d\n",numffts);
#endif
    ...
    fplintf(stderr,"KISS\tnfft=");
    for (k=0;k<ndims;++k)
        fplintf(stderr, "%d,",nfft[k]);
    fplintf(stderr,"\tnumffts=%d\n" ,numffts);
    pstats_report();
