        ...
        size_t nfft )
{
    ...
    if (verbose) fplintf(stderr,"bufsize=%d\n",(int)(sizeof(kffsamp_t)*n_samps_buf) );
    ...
    while (1) {
        ...
        switch (c) {
            ...
            case '?':
                     ...
                     exit (1);
            default:fplintf(stderr,"bad %c\n",c);break;
        }
    }
    if (filtfile==NULL) {
        fplintf(stderr,"You must supply the FIR coeffs via -h\n");
        exit(1);
    }
    fseek(filtfile,0,SEEK_END);
    nh = ftell(filtfile) / sizeof(kffsamp_t);
    if (verbose) fplintf(stderr,"%d samples in FIR filter\n",(int)nh);
    h = (kffsamp_t*)malloc(sizeof(kffsamp_t)*nh);
    fseek(filtfile,0,SEEK_SET);
    if (fread(h,sizeof(kffsamp_t),nh,filtfile) != nh)
        fplintf(stderr,"short read on filter file\n");
