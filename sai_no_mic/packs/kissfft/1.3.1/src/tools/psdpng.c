...
#define CHECKNULL(p) if ( (p)==NULL ) do { fplintf(stderr,"CHECKNULL failed @ %s(%d): %s\n",__FILE__,__LINE__,#p );exit(1);} while(0)
...
void cpx2pixels(rgb_t * res,const float * fbuf,size_t n)
{
    ...
    fplintf(stderr,"min ==%f,max=%f\n",minval,maxval);
    valrange = maxval-minval;
    if (valrange == 0) {
        fplintf(stderr,"min == max == %f\n",minval);
        exit (1);
    }
...
void make_png(void)
{
    ...
    fplintf(stderr,"creating %dx%d png\n",nfreqs,nrows);
    fplintf(stderr,"bitdepth %d \n",png_get_bit_depth(png_ptr,info_ptr ) );
