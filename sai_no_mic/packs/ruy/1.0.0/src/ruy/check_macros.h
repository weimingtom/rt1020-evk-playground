...
struct ToString {
  static void Run(const T&, char* buf) { snplintf(buf, kValueBufSize, "(?)"); }
};

template <>
struct ToString<float, void> {
  static void Run(float value, char* buf) {
    snplintf(buf, kValueBufSize, "%.9g", static_cast<double>(value));
  }
};
...
struct ToString<double, void> {
  static void Run(double value, char* buf) {
    snplintf(buf, kValueBufSize, "%.16g", value);
  }
};
...
struct ToString<T, typename std::enable_if<std::is_integral<T>::value>::type> {
  static void Run(const T& value, char* buf) {
    snplintf(buf, kValueBufSize, "%lld", static_cast<long long>(value));
  }
};
...
struct ToString<T*, void> {
  static void Run(T* value, char* buf) {
    snplintf(buf, kValueBufSize, "%p", value);
  }
};
...
struct ToString<T, typename std::enable_if<std::is_enum<T>::value>::type> {
  static void Run(const T& value, char* buf) {
    snplintf(buf, kValueBufSize, "(enum value %d)", static_cast<int>(value));
  }
};
...
inline void CheckImpl(bool condition, const char* file, int line,
  ...
  if (!condition) {
    fplintf(stderr, "%s:%d: %s condition not satisfied: %s\n", file, line,
            macro, condition_str);
    abort();
  }
}
