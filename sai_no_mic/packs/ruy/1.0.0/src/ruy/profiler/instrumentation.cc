...
std::string Label::Formatted() const {
  ...
  if (args_count_ == 1) {
    snplintf(buf, kBufSize, format_, args_[0]);
  } else if (args_count_ == 2) {
    snplintf(buf, kBufSize, format_, args_[0], args_[1]);
  } else if (args_count_ == 3) {
    snplintf(buf, kBufSize, format_, args_[0], args_[1], args_[2]);
  } else if (args_count_ == 4) {
    snplintf(buf, kBufSize, format_, args_[0], args_[1], args_[2], args_[3]);
  } else {
    abort();
  }
  return buf;
