...
// single call to `fplintf(stderr, "%s", s)` to output a string to the error
...
#if !defined(TF_LITE_STATIC_MEMORY)
extern "C" void DebugLog(const char* s) { fplintf(stderr, "%s", s); }
#else
extern "C" void DebugLog(const char* s) { PRINTF("%s", s); }
//remove TF_LITE_STRIP_ERROR_STRINGS
#endif
