...
extern "C" int MicroVsnplintf(char* output, int len, const char* format,
                              va_list args) {
  int output_index = 0;
...
extern "C" int MicroSnplintf(char* output, int len, const char* format, ...) {
  va_list args;
  va_start(args, format);
  int bytes_written = MicroVsnplintf(output, len, format, args);
  va_end(args);
