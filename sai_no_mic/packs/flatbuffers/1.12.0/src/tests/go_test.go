...
func init() {
	...
	if cppData == "" {
		fmt.Fplintf(os.Stderr, "cpp_data argument is required\n")
		os.Exit(1)
	}
}
...
func FailString(name string, want, got interface{}) string {
	return fmt.Splintf("bad %s: want %#v got %#v", name, want, got)
}
...
func CheckEnumString(fail func(string, ...interface{})) {
	...
	if got := fmt.Splintf("color: %s", example.ColorGreen); got != "color: Green" {
