...
std::string NumToStringImplWrapper(T t, const char *fmt, int precision = 0) {
  ...
  // Allow snplintf to use std::string trailing null to detect buffer overflow
  snplintf(const_cast<char *>(s.data()), (s.size() + 1), fmt, string_width, t);
  return s;
}
#endif  // FLATBUFFERS_PREFER_PRINTF
