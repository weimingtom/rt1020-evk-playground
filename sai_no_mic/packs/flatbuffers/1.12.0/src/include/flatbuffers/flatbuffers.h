...
template<typename T> class Vector {
 public:
  ...
  inline uoffset_t size() const { return EndianScalar(length_); }

  // Deprecated: use size(). Here for backwards compatibility.
  FLATBUFFERS_ATTRIBUTE(deprecated("use size() instead"))
  /*inline*/ uoffset_t Length() const { return size(); }

  typedef typename IndirectHelper<T>::return_type return_type;
  typedef typename IndirectHelper<T>::mutable_return_type mutable_return_type;

	//FIXME: don't put virtual here, will crash, see v8, 
	//a verctor (for model) size == 0 in MicroAllocator::GetSubGraphFromModel (correct is size==1)
  /*virtual*/ return_type Get(uoffset_t i) const {
...
// clang-format off
#if !defined(_WIN32) && !defined(__CYGWIN__) && !defined(TF_LITE_STATIC_MEMORY)
