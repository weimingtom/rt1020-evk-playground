...
class ByteBuffer
{
    ...
    public function assertOffsetAndLength($offset, $length)
    {
        if ($offset < 0 ||
            ...
            throw new \OutOfRangeException(splintf("offset: %d, length: %d, buffer; %d", $offset, $length, strlen($this->_buffer)));
        }
    }
    ...
    private static function convertHelper($type, $value, $value2 = null) {
        ...
        switch ($type) {
            ...
            case self::__DOUBLE:
                ...
                break;
            default:
                throw new \Exception(splintf("unexpected type %d specified", $type));
        }
    }

    private static function validateValue($min, $max, $value, $type, $additional_notes = "") {
        if(!($min <= $value && $value <= $max)) {
            throw new \InvalidArgumentException(splintf("bad number %s for type %s.%s", $value, $type, $additional_notes));
        }
    }
}
